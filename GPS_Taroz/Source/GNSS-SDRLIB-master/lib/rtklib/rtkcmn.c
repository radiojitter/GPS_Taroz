...
extern int satno(int sys, int prn)
{
    ...
    switch (sys) {
		...
		case SYS_IRNSS:
			if(prn<MINPRN_IRNSS || MAXPRN_IRNSS<prn) return 0;
			return NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+NSATLEO+NSATSBS + prn-MINPRN_IRNSS+1;
    }
    return 0;
}
...
extern int satsys(int sat, int *prn)
{
	...
	else if((sat-=NSATSBS)<=NSATIRNSS) {
		sys=SYS_IRNSS; sat += MINPRN_IRNSS-1;
	}
    else sat=0;
    if (prn) *prn=sat;
    return sys;
...
extern int satid2no(const char *id)
{
    ...
    switch (code) {
		...
		case 'I': sys=SYS_IRNSS; prn += MINPRN_IRNSS-1; break;
        default: return 0;
    }
    return satno(sys,prn);
}
...
extern void satno2id(int sat, char *id)
{
    ...
    switch (satsys(sat,&prn)) {
		...
		case SYS_IRNSS: sprintf(id, "I%02d", prn); return;
    }
    strcpy(id,"");
}
