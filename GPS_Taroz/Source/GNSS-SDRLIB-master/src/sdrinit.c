...
extern void initacqstruct(int sys, int ctype, int prn, sdracq_t *acq)
{
	...
	if (ctype==CTYPE_IRNSS) acq->intg = ACQINTG_IRNSS;
...
extern int inittrkstruct(int sat, int ctype, double ctime, sdrtrk_t *trk)
{
	...
	if (ctype==CTYPE_IRNSS)	 trk->loop = LOOP_IRNSS;
...
extern int initnavstruct(int sys, int ctype, int prn, sdrnav_t *nav)
{
	...
	DEFINE_IRNSS_PREAMBLE_VAR(pre_irnss);
	...
	DEFINE_IRNSS_CODE_initnavstruct();
                     ...
                     sdrch_t *sdr)
{
	...
	SDRPRINTF("initsdrch(chno=%d,sys=%d, prn=%d,ctype=%d,dtype=%d,ftype=%d,f_cf=%g,f_sf=%g,f_if=%g,sdrch_t*=0x%x\n",
		chno,sys,prn,ctype,dtype,ftype,f_cf,f_sf,f_if,(int) sdr);
    int i;
    short *rcode;
